#!/usr/bin/env python3
"""INE Data Agent — extrae operaciones, tablas, datos y HVD vía API JSON."""
...
from src.python.config import BASE_DIR, INE_DIR, INE_API_BASE, HTTP_HEADERS, source_dirs
from src.python.log_utils import ScrapeLogger

log = ScrapeLogger("scrape_ine", "INE")

# Directorios
RAW_DIR  = source_dirs(INE_DIR)["raw"]
PROC_DIR = source_dirs(INE_DIR)["processed"]
OKF_DIR  = source_dirs(INE_DIR)["okf"]
CONCEPTOS_DIR = source_dirs(INE_DIR)["conceptos"]
HVD_DIR  = source_dirs(INE_DIR)["raw_hvd"]

for d in [RAW_DIR, PROC_DIR, OKF_DIR, CONCEPTOS_DIR, HVD_DIR]:
    d.mkdir(parents=True, exist_ok=True)

API_BASE = INE_API_BASE
...
def api_get(path):
    ...
    try:
        ...
        log.warning(f"HTTP {e.code} en {url}", http_code=e.code)
        return None
    except urllib.error.URLError as e:
        log.warning(f"Timeout/red en {url}: {e.reason}")
        return None
    except json.JSONDecodeError as e:
        log.warning(f"JSON inválido en {url}: {e}")
        return None
    except Exception as e:
        log.error(f"Error inesperado en {url}", exc_info=True)
        return None
